...
export function NoveltyPanel({ noveltyStats }: { noveltyStats: NoveltyStats[] }) {
	return (
		<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
			{noveltyStats.map((ns) => (
			<div key={ns.user} className="border border-border bg-background/40 p-3 space-y-2 min-w-0">
				...
				{ns.messages.length > 0 && (
					<div className="max-h-96 overflow-y-auto grid gap-x-2 gap-y-0.5 font-mono text-[0.65rem] min-w-0" style={{ gridTemplateColumns: "max-content max-content max-content 1fr" }}>
							{ns.messages.map((m, i) => (
								<Fragment key={i}>
